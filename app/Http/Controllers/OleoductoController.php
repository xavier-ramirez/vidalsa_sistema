...
class OleoductoController extends Controller
{
    ...
    public function addPuntoFrente(Request $request, $idFrente)
    {
        ...
        $o = MapaOleoducto::firstOrCreate(
            ['id_frente' => $frente->ID_FRENTE],
            ['nombre' => $frente->NOMBRE_FRENTE, 'color' => $data['color'] ?? '#00e5ff', 'creado_por' => auth()->id()]
        );
        ...
        return response()->json([
            'success'      => true,
            'oleoducto_id' => $o->id,
            // Si el proyecto se creó en esta llamada, se devuelve completo para dibujarlo.
            'oleoducto_nuevo' => $creado ? [
                'id' => $o->id, 'id_frente' => $o->id_frente, 'nombre' => $o->nombre,
                'color' => $o->color, 'recorrido' => $o->recorrido, 'puntos' => [],
            ] : null,
            'punto' => ['id' => $p->id, 'nombre' => $p->nombre, 'lat' => (float) $p->latitud, 'lng' => (float) $p->longitud, 'orden' => $p->orden],
        ]);
    }
    ...
    /** Borra un oleoducto completo (sus puntos caen por cascade). El frente NO se toca. */
