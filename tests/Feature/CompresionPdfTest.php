...
/**
 * Compresion nocturna de PDF (docs:comprimir, CompresorPdf y la pantalla del registro).
 *
 * Lo que fijan: que la tarea NO cambie documentos donde la base es local (el PC de
 * desarrollo comparte Google Drive con el servidor pero no la base), que el cambio de
 * enlace no pise un documento reemplazado y arrastre las correcciones anexas, que la
...
{
    public function test_solo_se_activa_donde_la_base_es_la_del_servidor(): void
    {
        config(['services.compresion_pdf.nocturna' => null]);

        config(['database.connections.mysql.host' => '127.0.0.1']);
        $this->assertFalse(ComprimirDocumentos::activadaAqui()[0], 'Con la base local no debe activarse.');
        config(['database.connections.mysql.host' => 'localhost']);
        $this->assertFalse(ComprimirDocumentos::activadaAqui()[0]);

        config(['database.connections.mysql.host' => 'sistemavidalsa_vidalsa2302']);
        $this->assertTrue(ComprimirDocumentos::activadaAqui()[0], 'Con la base del servidor debe activarse.');

        // La variable fuerza la respuesta en los dos sentidos.
        config(['services.compresion_pdf.nocturna' => false]);
        $this->assertFalse(ComprimirDocumentos::activadaAqui()[0]);
        config(['services.compresion_pdf.nocturna' => true, 'database.connections.mysql.host' => '127.0.0.1']);
        $this->assertTrue(ComprimirDocumentos::activadaAqui()[0]);
    }

    public function test_el_comando_no_cambia_documentos_con_la_base_local(): void
    {
        config(['services.compresion_pdf.nocturna' => null, 'database.connections.mysql.host' => '127.0.0.1']);
    ...
    public function test_el_principal_se_lleva_sus_correcciones_y_no_pisa_un_reemplazo(): void
    {
        ...
        $doc = ['tabla' => 'documentacion', 'pk' => 'ID_EQUIPO', 'col' => 'LINK_POLIZA_SEGURO',
                'fila' => $equipo, 'id' => 'VIEJO_PRUEBA_CPDF'];

        $this->assertTrue(ComprimirDocumentos::cambiarEnlace($doc, 'NUEVO_PRUEBA_CPDF'));
        ...
        // Ya no apunta al viejo: un segundo intento (o un reemplazo hecho mientras tanto)
        // no debe tocar nada.
        $this->assertFalse(ComprimirDocumentos::cambiarEnlace($doc, 'OTRO_PRUEBA_CPDF'));
        $this->assertStringStartsWith('/storage/google/NUEVO_PRUEBA_CPDF?v=',
            DB::table('documentacion')->where('ID_EQUIPO', $equipo)->value('LINK_POLIZA_SEGURO'));
    }
    ...
    public function test_la_correccion_anexa_cambia_su_enlace_y_su_id(): void
    {
        ...
        $this->assertTrue(ComprimirDocumentos::cambiarEnlace(
            ['tabla' => 'documento_anexos', 'pk' => 'ID_ANEXO', 'col' => 'LINK', 'fila' => $anexo, 'id' => 'ANEXO_VIEJO_CPDF'],
            'ANEXO_NUEVO_CPDF'
        ));
