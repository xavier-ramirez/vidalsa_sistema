...
(function () {
    ...
    function _buildSearcher(prefix) {
        ...
        return function (q) {
            ...
            timer = setTimeout(() => {
                ...
                fetch('{{ route("fallas.searchActivos") }}?q=' + encodeURIComponent(q), {
        ...
        .then(({status, body}) => {
            if (status === 200 && body.success) {
                ...
                if (body.falla && body.falla.TIPO_REPORTE === 'extenso' && body.falla.ID_FALLA) {
                    setTimeout(() => {
                        window.open('{{ url("admin/fallas") }}/' + body.falla.ID_FALLA + '/pdf', '_blank');
                    }, 300);
                }
            } else {
    ...
    window.submitCambioEstado = function () {
        ...
        if (window.showPreloader) window.showPreloader();
        fetch('{{ route("fallas.changeEstado") }}', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': csrf(), 'Accept': 'application/json' },
            body: fd
        })
