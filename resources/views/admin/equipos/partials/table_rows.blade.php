@forelse($equipos as $equipo)
    ...
    <tr>
        ...
        <td class="table-cell-custom" style="font-size: 13px; color: #000; word-wrap: break-word;">
            <div style="font-weight: 700; text-transform: uppercase; line-height: 1.25;">
                {{ $equipo->tipo->nombre ?? '—' }}
            </div>
            @if($equipo->NUMERO_ETIQUETA)
                <div style="margin-top: 4px;">
                    <span style="display:inline-flex; align-items:center; gap:3px; font-size:10.5px; font-weight:700; color:var(--maquinaria-blue); background:#e0f2fe; border:1px solid #bae6fd; padding:1px 6px; border-radius:6px; letter-spacing:0.3px;">
                        <i class="material-icons" style="font-size:11px;">tag</i>{{ $equipo->NUMERO_ETIQUETA }}
                    </span>
                </div>
            @endif
            @if($equipo->CATEGORIA_FLOTA)
                <div style="font-size: 10.5px; color: #64748b; font-weight: 600; text-transform: uppercase; margin-top: 4px; letter-spacing: 0.3px;">
                    {{ $equipo->CATEGORIA_FLOTA }}
                </div>
            @endif
        </td>

        {{-- 3. MARCA / MODELO ─ misma jerarquia que aux: marca UPPERCASE bold,
             modelo debajo en gris. Año en linea muteada al final. --}}
        <td class="table-cell-custom" style="font-size: 13px; color: #000; word-wrap: break-word;">
            <div style="font-weight: 700; text-transform: uppercase; line-height: 1.25;">
                {{ $equipo->MARCA ?: '—' }}
            </div>
            @if($equipo->MODELO)
                <div style="font-size: 13px; color: #64748b; font-weight: 500; text-transform: uppercase; margin-top: 3px; line-height: 1.25;">
                    {{ $equipo->MODELO }}
                </div>
            @endif
            @if($equipo->ANIO)
        ...
        {{-- 4. SERIALES / PLACA / ID ─ 4 lineas compactas (12.5px) con labels
             muteados estilo aux. La placa se destaca en azul cuando existe. --}}
        <td class="table-cell-custom" style="font-size: 12.5px; color: #4a5568;">
            <div style="line-height: 1.35; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
            ...
            @if($equipo->SERIAL_DE_MOTOR)
                <div style="line-height: 1.35; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
            ...
            @else
                <div style="line-height: 1.35; margin-top: 2px;">
                    <strong style="color:#64748b;">P:</strong>
