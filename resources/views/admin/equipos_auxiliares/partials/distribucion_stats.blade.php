@php
    $showFrentes = $showFrentes ?? false;
@endphp

@if($showFrentes)
    @php
        $dist = $distribucionFrentes ?? collect();
    ...
    <ul style="list-style:none;padding:0;margin:0;max-height:50vh;overflow-y:auto;display:flex;flex-direction:column;gap:4px;" class="custom-scrollbar">
        @foreach($dist as $row)
            ...
            <li onclick="window.auxMainSelect('frente', '{{ $row->ID_FRENTE_ACTUAL ?? 'none' }}', '{{ addslashes($row->NOMBRE_FRENTE ?? 'Sin Asignar') }}'); window.cargarAuxiliares();"
                ...
                <div style="display:flex;justify-content:space-between;margin-bottom:2px;gap:4px;">
                    <span style="color:#334155;font-size:12.5px;font-weight:600;line-height:1.25;flex:1;text-transform:uppercase;">
                        {{ mb_strtoupper($row->NOMBRE_FRENTE ?? 'Sin Asignar') }}
                    </span>
                ...
                <div style="width:100%;height:4px;background:#e2e8f0;border-radius:2px;overflow:hidden;">
                    <div style="width:{{ $pct }}%;height:100%;background:linear-gradient(90deg,#10b981 0%,#059669 100%);"></div>
                </div>
            </li>
        @endforeach
    </ul>
        ...
        <ul style="list-style:none;padding:0;margin:0;max-height:50vh;overflow-y:auto;display:flex;flex-direction:column;gap:4px;" class="custom-scrollbar">
            @foreach($dist as $row)
                ...
                <li onclick="window.auxFilterByTipo('{{ $row->TIPO }}')"
                    ...
                    <div style="display:flex;justify-content:space-between;margin-bottom:2px;gap:4px;">
                        <span style="color:#334155;font-size:12.5px;font-weight:600;line-height:1.25;flex:1;text-transform:uppercase;">
                            {{ mb_strtoupper($tiposLabel[$row->TIPO] ?? $row->TIPO) }}
                        </span>
                        <span style="font-weight:700;color:#1e293b;font-size:12.5px;background:#f1f5f9;padding:2px 8px;border-radius:4px;white-space:nowrap;">
                            {{ $row->total }}
                        </span>
