 ...
 * <script src> ya cargado, asi que es el Blade quien llama a recepcionEntradaArrancar(CFG)
 * cada vez que se monta la pantalla. Lo que cambia entre una apertura y otra —rutas,
 * permisos y catalogos— llega en ese CFG.
 */
window.recepcionEntradaArrancar = function (RECE_CFG) {
    RECE_CFG = RECE_CFG || {};
(function () {
    'use strict';
    ...
    (function () {
        window.apiFetch(RECE_CFG.rutaAlmacenProductosAutocomplete, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
    ...
    var UNIDADES_MEDIDA = RECE_CFG.unidadesMedida;
    ...
    var SEPARA_PROYECTOS  = RECE_CFG.separaProyectos;
    var ID_FRENTE_DESTINO = RECE_CFG.idFrenteDestino;
    ...
    function entDocAbierto() { var o = el('entDocOverlay'); return !!o && !o.hidden; }
    // Un solo aviso a la vez: dentro del modal de los datos del documento si está abierto
    // (ahí salen los errores del registro) y, si no, en el panel de captura.
    function showErr(msg) {
        ['entError', 'entDocError'].forEach(function (id) {
            var e = el(id); if (e) { e.style.display = 'none'; e.textContent = ''; }
        });
        var t = msg ? el(entDocAbierto() ? 'entDocError' : 'entError') : null;
    ...
    // SPA: la función de arranque de este archivo vuelve a correr en cada visita a la
    // página, así que este listener de `document` se registraría otra vez cada vez.
    //
    ...
    // Flechas ↑↓ del teclado sobre el campo de cantidad: suman/restan 1 al valor (ver el
    // keydown de arriba, que es su único llamador — el stepper no tiene botones dibujados).
    // Si el campo está vacío o no es numérico, arrancan en 1. El mínimo es 0 (queda en blanco).
    ...
    // La UM ya viene del campo #entUm del panel de captura (entAgregar la lee y
    // la pasa). Antes esto se hacia via un mini-modal aparte — removido por
    // redundancia ahora que UM esta in-line en el panel.
    ...
    function entDoCreateProducto(nombre, cant, um, categoria) {
        ...
        // final (entGuardar / "Registrar" del modal). El guard entCreandoProducto ya
    ...
    window.entAgregar = function () {
        ...
        // en un dialogo aparte — ahora la UM esta in-line en el panel de captura.
    ...
    // Resumen del panel: cuántos productos y las cantidades sumadas POR unidad de medida
    // (en el orden en que aparecen en la tabla). Lo pinta entRender, el único punto que
    ...
    function entResumen() {
        ...
        entLineas.forEach(function (l) {
            var u = l.um || 'UND';
            if (!(u in porUm)) { porUm[u] = 0; orden.push(u); }
            porUm[u] = +(porUm[u] + l.cantidad).toFixed(3);
        });
        box.innerHTML = orden.length
            ? orden.map(function (u) { return '<span class="ent-res-chip">' + escHtml(fmtCant(porUm[u])) + ' <small>' + escHtml(u) + '</small></span>'; }).join('')
            : '—';
    }
    function entRender() {
    ...
    // ── Registrar: modal de los datos del documento → POST ───────────────
    // "Registrar entrada" del panel: con al menos una línea, abre el modal. El proyecto (si
    // hace falta y no se eligió) o la nota de entrega reciben el foco.
    window.entAbrirDocumento = function () {
        // Un producto nuevo todavía creándose (entDoCreateProducto) aún no está en la tabla:
        // registrar ahora lo dejaría fuera, y su aviso o su foco caerían en el modal.
        if (entCreandoProducto) return;
        showErr('');
        if (!v('entAlmacen')) { var mAlm = 'No se pudo determinar el almacén destino. Recarga la página o avisa al administrador.'; showErr(mAlm); toast(mAlm, 'error'); return; }
    ...
    // ✕ / Cancelar / Escape del modal: solo lo cierra; lo escrito se conserva.
    window.entCerrarDocumento = function () {
        var o = el('entDocOverlay'); if (!o || o.hidden) return;
        showErr('');
        o.hidden = true;
    };
    ...
    window.entGuardar = function () {
        ...
        .then(function (res) {
            ...
            if (res.ok) {
                // Exito: cerramos el modal, vaciamos el borrador y nos quedamos en el
                // modulo, listos para la siguiente entrada (flujo tipo POS). Antes redirigia
                // a la bandeja; el cliente pidio que el boton solo notifique, sin navegar.
    ...
    // datos del documento del modal (proyecto/nota/proveedor/fecha). Lo reusan
    // "Cancelar" del panel y el EXITO del registro: en ambos casos el modulo NO
    ...
    function entLimpiarTodo() {
        entLineas = [];
        entRender();
        // Reset del panel de captura (buscador + badge + UM). El `true` suprime
    ...
    // ── Cancelar (panel): vacia el borrador SIN salir del modulo ──
    window.entCancelar = function () {
        ...
        // confirmarAccion (layout_ui.js) ya trae el respaldo al confirm() del navegador
        // para cuando el modal no está montado: no hay que repetir ese if/else aquí.
