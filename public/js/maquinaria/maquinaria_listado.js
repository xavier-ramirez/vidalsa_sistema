 ...
 * <script src> ya cargado, asi que es el Blade quien llama a maquinariaArrancar(CFG)
 * cada vez que se monta la pantalla. Lo que cambia entre una apertura y otra —rutas,
 * permisos y catalogos— llega en ese CFG.
 */
window.maquinariaArrancar = function (MAQ_CFG) {
    MAQ_CFG = MAQ_CFG || {};
(function () {
    ...
    window._auxScrollState = {
        offset: MAQ_CFG.nextOffset,
        hasMore: MAQ_CFG.hasMore,
        loading: false
    };
    ...
    window.cargarAuxiliares = function (opts) {
        ...
        return window.apiFetch(MAQ_CFG.rutaEquiposAuxiliaresIndex + '?' + params.toString(), { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => r.json())
        .then(data => {
    ...
    (function () {
        ...
        try {
            ...
            params.set('offset', '0');
            window.apiFetch(MAQ_CFG.rutaEquiposAuxiliaresIndex2 + '?' + params.toString(), { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
    ...
    window.auxSubmitMovilizar = function () {
        ...
        const bulkMoveUrl = MAQ_CFG.rutaEquiposAuxiliaresBulkMove;
    ...
    window.exportAuxiliaresXlsx = function () {
        ...
        const url = MAQ_CFG.rutaEquiposAuxiliaresExport + (params.toString() ? '?' + params.toString() : '');
    ...
    window.openAuxAnclarBulkModal = function () {
        ...
        const fetchAndRender = (urlParams, isRecommend) => {
            ...
            window.apiFetch(MAQ_CFG.rutaEquiposAuxiliaresSearchHosts + '?' + urlParams, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => r.json())
                .then(rows => {
                    if (!rows || !rows.length) {
    ...
    window.openAuxAnclajesModal = function () {
        ...
        if (!modal) {
            ...
            document.getElementById('btnAuxAnclajesExport').addEventListener('click', function () {
                ...
                var url = MAQ_CFG.rutaEquiposAuxiliaresExportAnclajes + (qs ? ('?' + qs) : '');
                if (typeof window.showPreloader === 'function') window.showPreloader();
                window.apiFetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        ...
        var _urlIn = MAQ_CFG.rutaEquiposAuxiliaresAnchoredList + (_qsIn ? ('?' + _qsIn) : '');
